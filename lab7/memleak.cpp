...
{
  int sum = 0;
  for(int i=0; i < len; i++){ //CHANGED <= to < (wont access 
    //out of bounds index!!!)
    sum += data[i];
  }
  return sum;
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0';  //DONT DELETE WORD
  //DONT ADD +1 to LEN TO AVOID OUT OF BOUNDS ACCESS!!
...
int main(int argc, char* argv[])
{
  ...
  if(test == 1){
    ...
    delete [] myword;    //DELETES MYWORD
    delete [] otherword;  //DELETS OTHERWORD
  }
  else {
